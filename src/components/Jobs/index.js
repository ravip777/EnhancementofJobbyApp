...
class Jobs extends Component {
  state = {
    ...
    minimumSalary: 0,
    searchInput: '',
    locations: [], // Add locations to state
  }
  ...
  getJobs = async () => {
    ...
    const apiUrl = `https://apis.ccbp.in/jobs?employment_type=${employeeType.join()}&minimum_package=${minimumSalary}&location=${locations.join()}&search=${searchInput}`
  ...
  changeLocation = (locationId, isChecked) => {
    if (isChecked) {
      this.setState(
